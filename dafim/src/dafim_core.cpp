...
//
// 说明：
//   - 本版本：Baseline、AAFER-orig 使用“原始 RR 覆盖贪心”；DAFIM 使用
//       score = cnt * exp(-lambda_homo * r_part[p])
//     且三种方法都“强制选满 K”，即使 cnt 全部为 0；fallback 也用同一套 score 逻辑。
//   - METIS-hybrid: 使用原始 METIS 分区（不做 AAFER）+ HASR based partition reweighting
...
struct Args {
    ...
    i64   n = 0;             // 会在读图后被覆盖
    int   metis_parts = 32;  // METIS 分区数（≥ ranks）
    double p_default = 0.01; // IC 概率
    int   ic_trials = 500;   // IC 蒙特卡洛次数
    int   seed = 42;         // 随机种
    bool  use_ghost = false; // Stage-1 是否把 ghost 计入 RR（但不扩展）
    string scheme = "inv";   // AAFER: "inv" 或 "exp"
    double alpha = 1e-12;
    int theta2 = 200000;     // Stage-2 RR 总数（>0 生效；<=0 则自动启发式）
    double lambda_homo = 2.0; // homophily-based 加权的 λ
} AR;
...
struct GGlobal {
    i64 n = 0;
    vector<int> group;   // 节点群体标签（0/1）
    vector<int> xadj;    // CSR (无向展开) for METIS & RR
    vector<int> adjncy;
    vector<int> adjwgt;  // AAFER 权重 -> METIS 边权（初始=1）
    vector<float> prob;  // IC 概率（统一 p_default）
};

/************ 真实图加载：Rel + Att (Facebook-like) ************/
// Rel.txt:   "u v"
// Att.txt:   自定义格式：nid c2 c3 c4 c5 a6 c7
//            第6列 a6 > 2008 → group 1
//            第6列 a6 <=2008 → group 0
...
static void run_aaf_rank0(GGlobal& GG, const string& scheme, double alpha) {
    ...
    vector<array<int,4>> cnt(n); // cnt[u][g]，这里最多用到 g=0,1
    for(i64 u=0;u<n;u++){
...
// --------- METIS: 只在 rank0 调用，parts<=1 走 trivial ----------
...
struct LocalGraph {
    int n_local=0, n_total=0;          // 本地真实顶点数；含 ghost 的总本地 id 数
    vector<int> owner;                 // local-id -> owner rank
    vector<int> gid_of;                // local-id -> global id
    unordered_map<int,int> lid_of;     // global id -> local-id
    vector<int> rowptr;                // 仅对本地真实顶点：len=n_local+1
    vector<int> col;                   // 邻接（local-id），可指向 ghost
    vector<float> prob;                // 概率，与 col 对齐
    vector<int> group;                 // local-id 的 group（ghost 简单继承）
};
...
{
    (void)part_rank0; // 当前散列仅基于 owner_of_gid
    double t0=now();
...
static unordered_map<int,long long> allreduce_map_sum(const unordered_map<int,int>& mp){
    ...
    return global; // 只在 rank0 上有意义
}

// ================= Assortativity helpers =================

// 根据 E00, E11, E01 计算 assortativity（binary group: 0/1）
static double assort_from_counts(long long E00, long long E11, long long E01){
    ...
    // e[0][1] = e[1][0] = (double)E01 / (2.0 * (double)m);  // 跨组边除以 2*m
...
// 1) Original graph（所有边）assortativity
...
// 2/3) After partition: 只保留 intra-partition 边，整体当一张图
static double compute_assortativity_intra_global(
        const GGlobal& GG,
        const vector<int>& part,
        int nparts
){
    (void)nparts; // 这里只需要 part 本身
    long long E00 = 0, E11 = 0, E01 = 0;
...
// 4) 每个 partition 单独当一张子图的 assortativity + seeds in table

// 计算每个 partition 的 assortativity r_part[p]
...
static vector<int> distributed_greedy_select_fillK(
    ...
    bool prefer_rr_gain                  // true: 先用 cnt>0 ；false: 全程只看 w_part
){
    ...
    auto global_cnt = allreduce_map_sum(local_cnt0); // raw 覆盖计数，只在 rank0 有意义

    vector<int> S; S.reserve(K);
    vector<char> selected;  // 仅在 rank0 使用
    if(RANK==0) selected.assign((size_t)AR.n, 0);

    // 所有节点作为 fallback 候选（不限制在 C_global 里）
    ...
    for(int it=0; it<K; ++it){
        ...
        if(RANK==0){
            ...
            for(int u : all_nodes){
                ...
                // 如果我们要求先用 RR 增益，则在有正 cnt 时跳过 cnt=0 的点
                ...
                if(prefer_rr_gain && any_positive){
                    ...
                    // fallback：全是 cnt=0；只用 w 排序（baseline / orig 时 w=1）
                    score = w;
                }
        ...
        if(u_star < 0) break;  // 理论上只有 AR.n < K 时可能出现

        S.push_back(u_star);

        // 覆盖 RR 集合 & 更新 global_cnt（仍使用 raw 计数）
...
// ---------------- Sequential RR-set IM on rank0 (也强制选满 K) ----------------
    ...
    FastRng& rng, vector<int>& seeds_out, double& t_s2)
{
    ...
    for(int it=0; it<K; ++it){
        ...
        if(u_star<0) break; // 理论上只有 K > n 时才会发生
